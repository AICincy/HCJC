...
def test_dispatch_points_geocoding():
    from web.dispatch import _dispatch_points
    
    # 1. Valid CFS row inside Cincinnati box
    cfs_valid = {
        "latitude_x": "39.10692",
        "longitude_x": "-84.56740",
        "disposition_text": "CIT: CITED",
        "address_x": "34XX PRICE AV",
        "cpd_neighborhood": "EAST PRICE HILL",
        "create_time_incident": "2026-06-12T01:01:06.000",
    }
    # 2. Valid shooting row inside Cincinnati box
    shooting_valid = {
        "latitude_x": "39.15849",
        "longitude_x": "-84.47319",
        "type": "SHOOTING",
        "streetblock": "41XX READING RD",
        "sna_neighborhood": "PADDOCK HILLS",
        "datetimeoccured": "2026-06-11T23:06:32.000",
    }
    # 3. Valid shooting row with fallback datetime and neighborhood
    shooting_fallback = {
        "latitude_x": "39.12180",
        "longitude_x": "-84.52667",
        "streetblock": "4XX W MCMICKEN AV",
        "community_council_neighborhood": "OVER-THE-RHINE",
        "dateoccurred": "2026-06-11T23:46:08.000",
    }
    # 4. Out-of-bounds row (should be dropped)
    cfs_out_of_bounds = {
        "latitude_x": "45.00000",
        "longitude_x": "-90.00000",
        "disposition_text": "CIT: CITED",
    }
    # 5. Missing coord row (should be dropped)
    cfs_missing_coord = {
        "disposition_text": "CIT: CITED",
    }
    # 6. Malformed coord row (should be dropped)
    cfs_malformed_coord = {
        "latitude_x": "not-a-float",
        "longitude_x": "-84.56740",
    }
    # 7. Out-of-bounds shooting row (should be dropped)
    shooting_out_of_bounds = {
        "latitude_x": "45.00000",
        "longitude_x": "-90.00000",
        "type": "SHOOTING",
    }

    cfs_rows = [cfs_valid, cfs_out_of_bounds, cfs_missing_coord, cfs_malformed_coord]
    shooting_rows = [shooting_valid, shooting_fallback, shooting_out_of_bounds]

    pts = _dispatch_points(cfs_rows, shooting_rows)
    assert len(pts) == 3

    # Verify first point (CFS)
    assert pts[0]["la"] == 39.10692
    assert pts[0]["lo"] == -84.56740
    assert pts[0]["k"] == "cfs"
    assert pts[0]["d"] == "CIT: CITED"
    assert pts[0]["a"] == "34XX PRICE AV"
    assert pts[0]["n"] == "EAST PRICE HILL"
    assert pts[0]["t"] == "2026-06-12T01:01:06.000"

    # Verify second point (Shooting)
    assert pts[1]["la"] == 39.15849
    assert pts[1]["lo"] == -84.47319
    assert pts[1]["k"] == "shooting"
    assert pts[1]["d"] == "SHOOTING"
    assert pts[1]["a"] == "41XX READING RD"
    assert pts[1]["n"] == "PADDOCK HILLS"
    assert pts[1]["t"] == "2026-06-11T23:06:32.000"

    # Verify third point (Shooting with fallbacks)
    assert pts[2]["la"] == 39.12180
    assert pts[2]["lo"] == -84.52667
    assert pts[2]["k"] == "shooting"
    assert pts[2]["d"] == "SHOOTING"
    assert pts[2]["n"] == "OVER-THE-RHINE"
    assert pts[2]["t"] == "2026-06-11T23:46:08.000"


def test_dispatch_points_limit():
    from web.dispatch import _dispatch_points
    cfs_row = {
        "latitude_x": "39.10692",
        "longitude_x": "-84.56740",
    }
    pts = _dispatch_points([cfs_row] * 10, [], limit=5)
    assert len(pts) == 5
