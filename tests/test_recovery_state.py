...
* ``state-schema.md`` and ``cross-agent-state-table.md`` are archived stubs.
  Vocabulary and transitions live in this test file. Do not reload those
  packets as current schema.
...
def test_dashboard_agreement() -> None:
    ...
    assert text.startswith("# ARCHIVED \u2014 not current")
    assert "AUDIT-CONTRACT.md" in text
...
def test_schema_document_vocabularies_match_constants() -> None:
    ...
    assert text.startswith("# ARCHIVED \u2014 not current")
    assert "AUDIT-CONTRACT.md" in text
    vocabs, _ = parse_schema_doc(text)
    assert vocabs == {}
...
def test_schema_document_transitions_match_constants() -> None:
    text = SCHEMA_DOC.read_text(encoding="utf-8")
    assert text.startswith("# ARCHIVED \u2014 not current")
    _, doc_transitions = parse_schema_doc(text)
    assert doc_transitions == {}
