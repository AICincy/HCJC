...
def test_parse_charges_ignores_completely_empty_row(caplog):
    import logging
    caplog.set_level(logging.WARNING, logger="scraper.parsers")
    html = """
    <html><body>
      <h1>DOE, JANE</h1>
      <table>
        <thead>
          <tr>
            <th>Common Pleas Case #</th><th>ORC Code</th><th>Description</th>
            <th>Bond Type</th><th>Bond Amount</th><th>Court Date</th>
          </tr>
        </thead>
        <tbody>
          <tr>
            <td label="Common Pleas Case #"></td>
            <td label="ORC Code"></td>
            <td label="Description"></td>
            <td label="Bond Type"></td>
            <td label="Bond Amount"></td>
            <td label="Court Date"></td>
          </tr>
        </tbody>
      </table>
    </body></html>
    """
    inm, _, _ = parse_detail_page(html, "1234567")
    assert len(inm.charges) == 0
    # Ensure there are no warnings emitted
    assert not any("charge table parser found" in r.message for r in caplog.records)


def test_parse_charges_tracks_empty_but_present_high_value_labels(caplog):
    # Verify that if a high-value label (like Court Date) is present as an attribute
    # but empty across all rows, it does not trigger the "renamed column" warning.
    import logging
    caplog.set_level(logging.WARNING, logger="scraper.parsers")
    html = """
    <html><body>
    ...
    inm, _, _ = parse_detail_page(html, "1234567")
    assert len(inm.charges) == 1
    assert inm.charges[0].court_date == ""
    # Ensure there are no warnings emitted about missing high-value labels
    assert not any("absent from every row on a detail page" in r.message for r in caplog.records)


def test_parse_detail_page_handles_empty_photo_placeholder(caplog):
    import logging
    caplog.set_level(logging.INFO, logger="scraper.parsers")
    html = """
    <html><body>
      <h2>DOE, JANE</h2>
      <img src="data:image/png;base64," style="width:274px;">
    </body></html>
    """
    inm, photo, photo_url = parse_detail_page(html, "1234567")
    assert photo is None
    assert photo_url is None
    # Verify that the placeholder was identified and the drift message was suppressed
    assert any("no photo present (empty placeholder found)" in r.message for r in caplog.records)
    assert not any("investigate HTML drift" in r.message for r in caplog.records)
