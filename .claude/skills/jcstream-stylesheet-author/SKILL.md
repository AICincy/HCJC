...
Exception: the recent-bookings photo grid `.rb-grid` ships with its own non-720 breakpoints at `style.css` (`@media (max-width: 1080px)` → 3 cols, `@media (max-width: 540px)` → 2 cols) because the 6-col photo strip needs intermediate stops. There is also a sanctioned tablet breakpoint, `@media (max-width: 1024px)` (six blocks: court hero, caselaw rows, CFS list, inmate hero, statute hero, ladder grid), and one small-phone block, `@media (max-width: 480px)` ("Small phones": masthead padding, 1-col bio grid). These are the only non-720 widths in the file — don't take that as license to add more.
